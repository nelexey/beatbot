...
from bot.states.free_option import Find_Key
from bot.keyboards.inline import KB_GO_FREE_OPTIONS
from bot.misc.messages import FIND_KEY_MESSAGE
...
find_key_router = Router()


@find_key_router.callback_query(F.data == 'options:find_key')
async def find_key(c: CallbackQuery,
                   state: FSMContext,
                   user):
    ...
    await state.set_state(Find_Key.audio)
    await c.message.edit_text(FIND_KEY_MESSAGE, reply_markup=KB_GO_FREE_OPTIONS)


@find_key_router.message(Find_Key.audio)
async def get_audio(message: Message,
                    state: FSMContext,
                    user):
    if is_supported_format(message, 'find_key'):
        ...
        await free_option_req('find_key', message.chat.id, save_path)
