...
from bot.states.free_option import Speed_Up
from bot.keyboards.inline import KB_GO_FREE_OPTIONS
from bot.misc.messages import SPEED_UP_MESSAGE
...
speed_up_router = Router()


@speed_up_router.callback_query(F.data == 'options:speed_up')
async def speed_up(c: CallbackQuery,
                   state: FSMContext,
                   user):
    ...
    await state.set_state(Speed_Up.audio)
    await c.message.edit_text(SPEED_UP_MESSAGE, reply_markup=KB_GO_FREE_OPTIONS)


@speed_up_router.message(Speed_Up.audio)
async def get_audio(message: Message,
                    state: FSMContext,
                    user):
    if is_supported_format(message, 'speed_up'):
        ...
        await free_option_req('speed_up', message.chat.id, save_path)
